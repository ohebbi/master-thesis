...
Initally, we wanted to include the figures to the right in the attempt of
...
\noindent gradient boost performs optimally at $13$ principal components with a mean F1 score of $0.96$. The relevant hyperparameters were the regularization term for logistic regression, which was set to $0.021$, and the maximum number of iterations as $400$. The decision tree used an maximum depth of $6$, where larger values increased the training accuracy but not any other metric. Random forest was set with maximum depth of $6$, and gradient boost was given $4$.
