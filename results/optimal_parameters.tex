...
\noindent we find that the uncertainty of the confidence interval increases for all HT- methods.
...
Gradient boost, on the other hand, experiences minor changes for a larger number of principal components, where the optimal number of components marked could be $50$ principal components less without any remarks to the model's metrics. Notably, only a few principal components yield almost $100\%$ training accuracy for gradient boost, while not showing any clear sign of overfitting. Similarly, logistic regression shows signs of almost a perfect classifier with high scores for all metrics.
